---
title: "Computational Single and Budded Yeast Area Measurement"
author: "Mallory Miller, Mary Miller"
date: ""
output: pdf_document
---
...
All of the outlines discovered by the Canny edge detection algorithm need to be filled in to find the areas of of the yeast. In order to do this, the image is searched row by row for the first black pixel it encounters.
...
However, we are still not done with this area. Next we need to search around the found area in order to see if there are any other black pixels nearby that would count as buds. If there are any black pixels within `MAX_BUDDING_DISTANCE` of any edge of the area we found, that black pixel is the next area to fill. This generates a list where every item in the list is a different area and every area is a list of coordinates.
...
As areas are accumulated by filling in outlines, each one is categorized as either a single yeast, budded yeasts, or a cluster. Single yeasts and budded yeasts are of interest while "clusters" are disregarded. 

The first step in categorizing a found set of areas is by filtering out any areas in the list that are too small, meaning they have a smaller pixel count than `IGNORE_ALL_SIZE` or one of the included areas was within `MAX_BUDDING_DISTANCE` of the edge of the image, meaning the reading could be incomplete or there could be another area outside of the frame of view that would be within budding distance.

After all areas have been validated
